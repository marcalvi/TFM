...
The method-level tables summarize behavior into AUPMC metrics and Mean Degradation Ratios (MDR). The metrics are: Baseline AUC, Train-time AUPMC, Train MDR, Test-time AUPMC, Test MDR, Best fixed-train AUPMC, and BFT MDR. MDR values are computed as the arithmetic mean of `baseline / performance` across the evaluated trajectory points, including the baseline point. Train-time and test-time degradation use the complete-data baseline `(train=0, test=0)`. Best fixed-train degradation uses the selected fixed-train baseline `(train=m_train*, test=0)`, because that trajectory measures degradation as test missingness increases after selecting one training missingness regime. Values below 1 are possible when a method improves over its trajectory baseline at enough points; values close to 1 indicate low average degradation. AUPMC and MDR values include bootstrap 95% confidence intervals in `method_level_metrics.csv`; metric rankings are still based only on the mean point estimates. For distillation methods, train missingness in both/best fixed-train settings refers to the student branch. The ranking table orders methods separately for each metric, making it useful for identifying methods that are globally strong, not only methods that win a single missingness cell.
...
- `method_level_metrics.csv`: Baseline AUC, Train-time AUPMC, Train MDR, Test-time AUPMC, Test MDR, Best fixed-train AUPMC, selected train missingness, BFT MDR, and bootstrap 95% CIs for AUPMC/MDR metrics.
...
- `best_fixed_train_curve.csv`: best fixed-train behavior over train-missingness settings, used for Best fixed-train AUPMC and BFT MDR.
...
MDR in survival analysis uses the same definition as classification: the arithmetic mean of `baseline / cindex` across trajectory points. Train-time and test-time curves use the complete-data C-index baseline, while best fixed-train curves use the selected fixed-train C-index at `test=0`. Rankings are based on mean point estimates, while bootstrap 95% CIs are reported for AUPMC and MDR values.
