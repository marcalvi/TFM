...
class RadiologyAttentionPooler:
    ...
    def _prepare_bags(self, df, labels_df, id_col, label_col, feature_cols):
        ...
        if not grouped:
            raise ValueError("RadiologyAttentionPooler received no train bags after patient alignment.")
        return grouped
    ...
    def fit(self, df_train, labels_df, id_col="patient", label_col="label"):
        ...
        if not self.feature_cols:
            raise ValueError("RadiologyAttentionPooler found no feature columns to pool.")
    ...
    def transform(self, df):
        ...
        if self.id_col not in df.columns:
            raise ValueError(f"Radiology dataframe must contain '{self.id_col}'.")
