...
class HealNetBinaryWrapper(nn.Module):
    """Binary-classification adapter around original HealNet.

    Inputs in this pipeline are per-modality tensors [B, D]. We map each
    modality to [B, 1, D] to match HealNet's expected tabular format.
    """
    ...
    def __init__(
        ...
        # Vectors are treated as one-axis modalities.
        self.model = HealNet(
    ...
    def forward(self, Xs, present_mask=None):
        ...
        for i, Xi in enumerate(Xs):
            ...
            # Match original missing-modality behavior at batch level:
            # if modality i is missing in ANY sample, skip that modality update
            # for the whole batch by passing None.
            if present_mask is not None:
                modality_present = present_mask[:, i]
                if torch.any(modality_present == 0):
                    tensors.append(None)
                    continue
